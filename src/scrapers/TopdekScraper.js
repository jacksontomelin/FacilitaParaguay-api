...
class TopdekScraper extends BaseScraper {
  constructor() { super('topdek-informatica'); this.baseUrl = 'https://topdekinformatica.com.br'; }
  async scrape() {
    const page = await this.createPage();
    try {
      await scrapeTxtList(this, page, `${this.baseUrl}/lista-txt`);
      const cats = ['notebooks','monitores','placas-de-video','processadores','memoria-ram','ssd','perifericos','gabinetes','fontes','coolers','redes'];
      for (const c of cats) {
        try {
  ...
  async scrapePage(page, url, categoryId) {
    ...
    while (hasMore && p <= 20) {
      try {
        await page.goto(p > 1 ? `${url}?page=${p}` : url, { waitUntil: 'domcontentloaded' }); await this.delay(2500);
